      ...
      [op] when op in @combinator_operations ->
        ...
        percent_operations? =
          ...
          |> Enum.all?(fn elem ->
            ...
            type in @percent_operations
          end)

        if percent_operations? and all_operations_valid?(list) do
          :ok
        else
  ...
  def valid_absolute_change_operation?(operation) do
    case Map.keys(operation) do
      ...
      [op] when op in @combinator_operations ->
        ...
        absolute_change_operations? =
          list
          |> Enum.all?(fn elem ->
            type = Map.keys(elem) |> List.first()
            type in @absolute_change_operations
          end)

        if absolute_change_operations? and all_operations_valid?(list) do
          :ok
        else
    ...
    case Map.keys(operation) do
      ...
      [op] when op in @combinator_operations ->
        ...
        absolute_value_operations? =
          ...
          |> Enum.all?(fn elem ->
            type = Map.keys(elem) |> List.first()
            type in @absolute_value_operations
          end)

        if absolute_value_operations? and all_operations_valid?(list) do
          :ok
        else
